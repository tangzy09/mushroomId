...
## 认菌训练重构（二期第二件，2026-09-08）

原来的答题完全绑在「进山采菌」的抽卡流程里，没有独立入口。现在拆成一套共用同一份题目/作答
UI（`page-quiz`）、但结局不同的两条路：`round.mode` 是 `'gacha'`（原有流程，答完抽卡）还是
`'training'`（答完只报个分回上一页，`advance()` 里一处分支，不碰抽卡逻辑一行）。

- **入口**：详情页「🧠 测一测」（单种速测，有熟练度时按钮上直接显示 ★☆☆ 这类星级）；
  「我的」新入口卡「认菌训练」→ 训练中心（`page-training`），顶部「今日一鱼」（按日期字符串哈希
  选一种，一天内不变），下面四张模式卡。
- **四种模式**（`js/game/app.js`，都是 `Quiz.pickQuestions`/`QUESTIONS` 之外的取题逻辑，
  复用同一套 `showQuestion()`/`answer()`）：
  - **范围闪卡**：题目来自 `Browse._facet().results()`——图鉴当前筛选到什么就考什么，不是快照，
    是实时读。
  - **易混对决**：全库里没有这个题型，是运行时现造的——从一个有 `lookalikes` 的种里选一个对方，
    两个选项（该种 vs 最像的近亲），反馈句就是 `lookalikeCard` 已经在用的那句差异句，两处读同一个
    数据源。
  - **每日 5 题**：`熟练度×10 − 见过×3 + 随机`，取分数最低（最生疏）的 5 种，全部是看图题；
    「见过」取 `Storage.has()`（收藏小游戏）或 `observationsFor().length>0`（真实见过）任一为真。
  - **错题本**：题源是 `Storage.wrongList()`，优先给看图题，找不到就退回该种任意类型的题。
- **数据（`js/core/storage.js`）**：`mastery: {entityId: 0..3}`、`lastQuiz: {entityId: date}`、
  `wrong: [entityId]`。`answer()` 里统一记账：**任何题型**答错都进错题本、答对就移出；
  **只有看图题和易混对决**会动熟练度（这条是刻意的——一道文字知识题答对答错，说明不了「认不认得出
  长相」）。星级 30 天没复习会掉一颗，`rollover()` 里顺带算，不用单独跑定时任务；掉的同时把
  `lastQuiz` 拨到今天，不会连续多天反复掉。
- **验收**：`test/verify_training.mjs` 23 项，反向测过（注释掉 training 分支确认「答完不进抽卡」
  这条断言会红）。踩过一个真实的随机性坑：测错题本时一整轮全是同一个种的题，中途用「随手点第一个
  选项」的方式作答，有概率蒙对把这个种从错题本移出去，导致后面的断言偶发失败——**测「一直在错题本
