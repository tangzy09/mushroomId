...
> **v0.4（2026-09-07）：改版为以查阅为主、游戏为辅的图鉴，一期 A 已落地。** 设计见
> `docs/superpowers/specs/2026-09-07-fieldguide-redesign-design.md`，实施计划见
> `docs/superpowers/plans/2026-09-07-fieldguide-phase-a.md`。下文 v0.3 的「收集游戏」定位已被取代。
>
> 版本：v0.3 · 日期：2026-09-03 · 状态：**MVP 已实现**（三项待决策已按 §0.2 的建议自行拍板并落地，见 §0.3）
> 基于 5 份并行调研（fishId 架构复用评估 / 蘑菇识别 app 竞品分析 / 物种数据库方案 / 玩法设计 / AI 拍照识别可行性），原文见 `docs/research/`。
> 本报告是「设计决策」的汇总；各节标注了依据来源（R1–R5）。
>
> 修订：v0.1 初稿；v0.2 补充非目标、待决策事项、题库可达性校验、MVP 验收清单、成功指标、术语表，并订正档期年份与题量口径；v0.3 记录 MVP 实现结果与设计变更。
...
| 物种规模 | 数据储备 **250 种**（清单已列出 253 种）；MVP 实际入库 **181 种**，V1.0 到 250 种。稀有度四档保留 60/30/8/2 与 20/50 保底。（R3, R4） |
...
MVP 已按本报告实现并可运行。三项待决策：D1 取 (a) 分阶段扩容、D3 暂不改动 fishId。
**D2 已从 (a) 升级到 (b)**：完整四段状态机上线，见下方「生长」行与 §11.7。

| 模块 | 状态 | 与设计的差异 |
|---|---|---|
| 物种数据 | ✅ **181 种**（common 67 / rare 72 / epic 32 / legend 10） | 高于 MVP 目标的 120，低于 V1.0 的 200 |
| 题库 | ✅ **1,165 道** | 四类结构化题（看图、食性、特征、易混淆）改为**由物种数据生成**，人工只写 trivia / cold_fact / myth_buster。这比原计划更好：改数据自动同步，且题库可达性可由构造保证 |
| 数据校验 | ✅ `test/check_data.py` | 含字段、枚举、措辞黑名单、学名格式、lookalikes 双向、题库可达性 |
| core 层 | ✅ storage / gacha / quiz | 领域词禁令由测试强制执行 |
| 测试 | ✅ 58 条纯函数测试 + 15 条存档测试 + 浏览器端 e2e 与截图判据 | 首轮跑出 3 个真 bug，后续又跑出 5 个（见 §11） |
| 主循环 | ✅ 选林地 → 答 5 题 → 抽卡 → 定植 / 分解 | 完整 |
| 菌菇园 | ✅ 侧视剖面、10 槽、昼夜、天气、浇水、起风、点击反应 | 完整。构图后来重做：槽位从地平线铺到近处并按纵深缩放，补树冠、雾、蕨草、两根木头与落地阴影；夜里整幅压冷色、荧光物种透回来（§11.8） |
| 生长 | ✅ **完整四段** `pin → young → mature → sporulate`，收孢子回到 mature | 30 分钟成幼菌、3 小时成熟；产孢计时从成熟起算、不叠加；浇水推进整条曲线的 8%。上线前这套一直是死的，见 §11.7 |
| 行为动画 | ✅ **14 / 14** | 每种行为都有各自的形态与反应：喷孢、荧光、触摸变蓝、胶质、成簇、自溶滴墨、引蝇、湿度开合、溅蛋、珊瑚分枝、展裙、架状、虫生。`hygro` 会读天气：雨后张开、晴天合拢 |
| 图鉴 | ✅ 筛选、锁定轮廓、详情、易混淆对照 | 完整 |
| 经济 | ✅ 孢子、腐殖质、合成、菌种库、每日菌篮 | 完整 |
| 每日任务 | ✅ 4 项含「认出 1 种毒菌」 | 完整 |
| 分享 | ✅ 菌卡 / 菌菇园 / 里程碑三种，水印强制 | 挑战好友未做 |
| 安全体系 | ✅ 首次协议、常驻横幅、食性脚注、急救卡、强制辨毒误区题、措辞校验 | 完整 |
| 存档迁移 | ✅ 导出 / 导入 `.spore` | AES-GCM 加密，导出全部 `mush_*` key，坏文件不会破坏现有存档（15 条测试） |
| 礼品码 | ❌ 未做 | 优先级低 |
| 英文版 | ❌ 未做 | V1.0 |
| 小游戏 | ❌ 未做 | V1.0 起按 §5.6 优先级 |

**最重要的设计变更：不使用照片。** 原计划用 Wikimedia / iNaturalist 的 CC 照片做「看图认菌」，但真实照片的筛选与署名是整个项目最长的板（§10 风险表列为「高」）。改为 `js/game/shroom-art.js` 依据每个物种的形态字段程序化绘制：菌盖轮廓、颜色、菌环、菌托、菌褶或菌管、鳞片、白点、变色斑。

这个改动同时解决了三件事：MVP 不再被素材阻塞；完全没有图片版权义务；**画出来的特征恰好就是题目考察的特征**，看图与答题指向同一组知识。代价是失去真实照片的辨识训练价值——V1.0 可以在详情页补充 CC 照片作为对照，但主循环不再依赖它。
...
写测试和跑浏览器验证时抓到的真问题，都已修复，记录在此以免重犯：

1. **存储在空 store 上会继承上一次会话的内存状态**（`core/storage.js`）。浏览器里一次加载只 init 一次所以看不出来，测试里连续 init 两次就暴露了。
2. **领域词泄漏进 core 层**：`gardenSlots` / `plant` / `dailyForays` 都写进了内核。改为 `slots` / `place` / `dailyRuns`，并加了一条 grep 测试守住。
3. **结算按钮可重复点击**，每点一次多抽一张卡。`advance()` 现在会锁住已结束的回合。
4. **新手赠礼只在首次同意免责声明时发放**，已同意过的用户打开就是一座空园子。改为「收集为空即补发」。
5. **题库有孤儿桶**：一道 trivia 的难度没有任何难度设置会请求，一个专家档请求的桶题库里没有。这正是 fishId 43% 死题的同一类问题，校验器首轮就抓到了。
6. **71 处单向的 `lookalikes` 引用**，已在源数据里补成双向。

第二轮（浏览器里逐屏截图验收时抓到的，前四条都是「测试全绿但产品是坏的」）：

7. **生长机制从来没跑起来过。** 存档写的是 `placedAt` / `lastYieldAt` / `boostMs`，读的是 `plantedAt` / `wateredMs` / `lastSporeAt`，三个名字全对不上，算出来永远是 `NaN`——**每一株永远停在菌蕾、浇水毫无作用、没有任何槽位产过孢子**，整个「明天回来收」的留存假设是空的。54 条测试当时全绿，因为测试是手搓字面量喂给读侧的，用的正是读侧那套错名字。修法不只是改名：`core.test.js` 现在**从 `Storage.place` 造记录再喂给 `World.growth`**，任一侧改字段名都会红。
8. **菌菇园十个槽位全挤在底部一条窄带里**，种满之后互相压成一堵看不清的墙，上面三分之二是空的。改为按纵深铺开并缩放。
9. **马勃与猴头菇悬在半空**：绘制时按「菌柄高 + 菌盖高」上移，而这两类根本不画菌柄。顺带发现「高个子物种从卡片框顶部溢出」，改为 `ShroomArt.heightOf` **量出**真实高度而不是按公式推。
10. **三个菌盖形状是死的**：`round` / `egg` / `trumpet` 在数据里写着，绘制代码没有对应分支，六个物种被静悄悄画成同一个圆包子。校验器现在从绘制代码里读出可画形状再比对数据。
11. **`growth(slot, cfg, 0)` 把 `now=0` 当成「没传」**（`now || Date.now()`），偷偷改用真实时钟。
12. **本地开发服务器**在浏览器中途断连时把日志埋在一段 traceback 下面（Windows 上的 `ConnectionResetError`）。

---

## 12. 顺带发现的 fishId 问题（建议另开任务修复）
