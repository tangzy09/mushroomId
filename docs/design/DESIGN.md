...
MVP 已按本报告实现并可运行。三项待决策按 §0.2 的建议执行：D1 取 (a) 分阶段扩容、D2 取 (a) 简化生长、D3 暂不改动 fishId。
...
| 测试 | ✅ 54 条纯函数测试 + 浏览器端 e2e | 首轮跑出 3 个真 bug（见 §11 下方） |
| 主循环 | ✅ 选林地 → 答 5 题 → 抽卡 → 定植 / 分解 | 完整 |
| 菌菇园 | ✅ 侧视剖面、10 槽、昼夜、天气、浇水、起风、点击反应 | 完整 |
| 生长 | ✅ 简化版 `pin → young → mature`（30 分钟）+ 每槽 24h 产孢 | 按 D2 的决定，四段状态机字段已预留 |
