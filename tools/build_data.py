...
QOUT = os.path.join(ROOT, "js/questions.gen.js")
EOUT = os.path.join(ROOT, "js/i18n_en.gen.js")
...
def main():
    ...
    # 英译字段是二期英文界面的数据层准备，现在的中文界面一个字都用不上——
    # 留在 MUSHROOM_DATA 里就是让每个用户白白多下一份不会显示的文本（166 种 × 5 字段约 85 KB）。
    # 拆进一个不被 index.html / sw.js 引用的独立生成文件，源数据（data/mushrooms.json）仍然完整保留这些字段。
    EN_FIELDS = ("idKeysEn", "habitatEn", "lookalikeNotesEn", "factEn", "quoteEn")
    i18n_en = {}
    species_out = []
    for m in species:
        m2 = dict(m)
        ...
        if en:
            i18n_en[m["id"]] = en
        species_out.append(m2)

    js = ["// GENERATED by tools/build_data.py — do not edit by hand.",
          "// Source of truth: data/mushrooms.json",
          "var MUSHROOM_DATA = %s;" % json.dumps(species_out, ensure_ascii=False, separators=(",", ":")),
          ""]
    qjs = ["// GENERATED by tools/build_data.py — do not edit by hand.",
           ...
           ""]
    ejs = ["// GENERATED by tools/build_data.py — do not edit by hand.",
           "// 二期英文界面的数据层准备：idKeysEn/habitatEn/lookalikeNotesEn/factEn/quoteEn，按 id 存放。",
           "// 不被 index.html 引用，现在不占任何用户的下载量；接 i18n 引擎时再挂载。",
           "var I18N_EN = %s;" % json.dumps(i18n_en, ensure_ascii=False, separators=(",", ":")),
           ""]
    os.makedirs(os.path.dirname(OUT), exist_ok=True)
    open(OUT, "w", encoding="utf-8").write("\n".join(js))
    open(QOUT, "w", encoding="utf-8").write("\n".join(qjs))
    open(EOUT, "w", encoding="utf-8").write("\n".join(ejs))
